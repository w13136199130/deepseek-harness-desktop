...
describe('published package surface', () => {
  it('registers both npm launcher names', () => {
    expect(manifest.name).toBe('deepseek-harness-desktop')
    expect(manifest.bin).toEqual({
      'deepseek-harness-desktop': 'lib/bin.js',
  ...
  it('exposes the Host plugin and desktop-owned client face', () => {
    ...
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: deepseek-harness-desktop')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: deepseek-harness-desktop/terminal')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: deepseek-harness-desktop/pnpm')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: deepseek-harness-desktop/profiles')
    expect(readFileSync(new URL('cordis.patch.yml', packageRoot), 'utf8')).toContain('name: deepseek-harness-desktop/updates')
  })
  ...
  it('installs Host command PATHs after the launch snapshot and before profile boot', () => {
    ...
    expect(main).toContain("'deepseek-harness-desktop: packaged pnpm runtime PATH'")
    expect(main).toContain("'deepseek-harness-desktop: packaged dsh runtime PATH'")
  ...
  it('separates unsigned smoke packaging from the signed macOS release', () => {
    ...
    expect(workspaceManifest.scripts?.['dist:mac']).toBe('yarn workspace deepseek-harness-desktop dist:mac')
    expect(workspaceManifest.scripts?.['dist:win'])
      .toBe('yarn workspace deepseek-harness-desktop dist:win')
