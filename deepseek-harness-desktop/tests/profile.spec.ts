...
describe('desktop profile composition', () => {
  ...
  it('projects advanced YAML settings into the Host and client Loader rows', () => {
    const home = temporaryHome()
    writeFileSync(join(home, 'settings.yaml'), 'deepseek-harness-desktop:\n  mode: advanced\n')
  ...
  it('reads JSON settings and defaults an absent desktop namespace to compatibility', () => {
    ...
    writeFileSync(path, JSON.stringify({ 'deepseek-harness-desktop': { mode: 'advanced' } }))
  ...
  it('rejects invalid settings roots, sections, modes, and YAML', () => {
    ...
    expect(() => desktopShellModeFromSettings({ 'deepseek-harness-desktop': true })).toThrow('settings must be a map')
    expect(() => desktopShellModeFromSettings({ 'deepseek-harness-desktop': { mode: 'glass' } })).toThrow(
      'must be "compatibility" or "advanced"',
    )

    const home = temporaryHome()
    const path = join(home, 'invalid.yaml')
    writeFileSync(path, 'deepseek-harness-desktop: [\n')
  ...
  it('pins the Windows browse picker and desktop pwsh provider without replacing process boundaries', () => {
    ...
    expect(rows).toContainEqual(expect.objectContaining({
      ...
      name: 'deepseek-harness-desktop/windows-pwsh-sandbox',
