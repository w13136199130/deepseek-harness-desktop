...
describe('packaged desktop runtime verification', () => {
  it.each([
    [
      'darwin',
      join('/build', 'DeepSeek Harness Desktop.app', 'Contents', 'Resources', 'app.asar'),
    ],
    [
      'win32',
  ...
  it('fails loud when a required package export cannot resolve from app.asar.unpacked', () => {
    ...
    const resolvePackage = vi.fn<PackageResolver>((specifier) => {
      if (specifier === 'deepseek-harness-desktop/profiles') {
    ...
    expect(() => verifyPackagedRuntime(
      ...
      `packaged runtime at ${unpackedRoot} cannot resolve required package export deepseek-harness-desktop/profiles`,
    )
  })
