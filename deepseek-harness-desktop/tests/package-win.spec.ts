...
function options(calls: CommandCall[], logs: string[] = []): WindowsPackageOptions {
  return {
    ...
    desktopRoot: 'C:\\repo\\deepseek-harness-desktop',
    commandShell: 'C:\\Windows\\System32\\cmd.exe',
    builderCli: 'C:\\repo\\node_modules\\electron-builder\\cli.js',
    verifier: 'C:\\repo\\deepseek-harness-desktop\\scripts\\verify-win-installer.ts',
  ...
  it('checks without credentials, builds an unsigned NSIS target, then verifies it', () => {
    ...
    expect(calls[0]).toEqual({
      ...
      args: [
        ...
        '/s',
        '/c',
        'corepack yarn workspace deepseek-harness-desktop check:win-package',
      ],
      cwd: 'C:\\repo',
    ...
    expect(calls[1]).toEqual({
      ...
      cwd: 'C:\\repo\\deepseek-harness-desktop',
      env: {
        PATH: 'C:\\Windows\\System32',
        SAFE_VALUE: 'kept',
    ...
    expect(calls[2]).toEqual({
      command: 'C:\\Program Files\\nodejs\\node.exe',
      args: ['C:\\repo\\deepseek-harness-desktop\\scripts\\verify-win-installer.ts'],
      cwd: 'C:\\repo\\deepseek-harness-desktop',
      env: { PATH: 'C:\\Windows\\System32', SAFE_VALUE: 'kept' },
    })
    expect(logs).toEqual([
