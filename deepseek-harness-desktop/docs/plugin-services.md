...
import type {
  ...
  DesktopProfiles,
} from 'deepseek-harness-desktop/profile-service'
import type {
  DesktopPnpm,
  DesktopPnpmHandle,
  DesktopPnpmOutcome,
} from 'deepseek-harness-desktop/pnpm'
```

`deepseek-harness-desktop/profiles` is the Desktop-owned tray consumer, not the profile service contract. Do not import it for service types.
...
| --- | --- | --- |
| `desktopProfiles` | Generation-scoped Host service. | Public and supported through `deepseek-harness-desktop/profile-service`. |
| `desktopPnpm` | Generation-scoped Host service. | Public and supported through `deepseek-harness-desktop/pnpm`. |
...
import type {} from 'deepseek-harness-desktop/profile-service'
import type { DesktopPnpmHandle } from 'deepseek-harness-desktop/pnpm'
...
import type { Context } from '@deepseek-ai/cordis'
import type {} from 'deepseek-harness-desktop/profile-service'
import type {} from 'deepseek-harness-desktop/pnpm'
...
Type-only imports are erased from JavaScript. A cross-environment package can keep `deepseek-harness-desktop` as a development dependency for compilation, or as an optional peer if it publishes declarations that expose these types; it does not need a runtime import merely to probe the services.
...
```sh
yarn workspace deepseek-harness-desktop build
yarn workspace deepseek-harness-desktop verify:profile
```
