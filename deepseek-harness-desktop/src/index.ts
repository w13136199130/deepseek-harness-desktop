/** DeepSeek Harness Desktop Host plugin: owns the selected native shell generation. */
...
export const DESKTOP_SETTINGS_NAMESPACE = settingsNamespace('deepseek-harness-desktop')
...
export function desktopRendererUrl(
  ...
  url.searchParams.set('deepseek-harness-desktop-mode', mode)
  url.searchParams.set('deepseek-harness-desktop-platform', platform)
  return url.href
}
...
export function apply(ctx: Context, config: Config): void {
  ...
  if (runtime === undefined) {
    process.stderr.write(
      'deepseek-harness-desktop: this profile is composed with the DeepSeek Harness Desktop shell, which requires the desktop launcher (desktopRuntime).\n'
      + 'Start it with `deepseek-harness-desktop`, or select this profile inside the packaged DeepSeek Harness Desktop application.\n'
  ...
  if (appExit === undefined) {
    throw new Error('deepseek-harness-desktop: the launcher did not provide ctx.appExit')
  }
  if (ctx.webServer.host !== '127.0.0.1') {
    throw new Error('deepseek-harness-desktop: desktop shell requires a loopback Web server')
  }
    ...
    DesktopSettingsSchema,
    {
      ...
      validate: (value) => {
        if (value.mode === 'advanced' && runtime.platform === 'linux') {
          throw new Error('deepseek-harness-desktop: advanced shell mode is supported on macOS and Windows')
        }
      },
    },
  )
  ...
  ctx.effect(
    ...
    'deepseek-harness-desktop: renderer boot report route',
  )
  ctx.effect(() => {
    ...
    const stopWatching = settings.watch((next) => {
      ...
      pending ??= setImmediate(() => {
        ...
        void runtime.requestRestart().catch((cause: unknown) => {
          ctx.logger.error('deepseek-harness-desktop: failed to restart after mode change')
  ...
  }, 'deepseek-harness-desktop: restart after mode change')
  ...
  ctx.effect(
    () => runtime.schedule({
      ...
      readThemeSource: () => {
        ...
        if (theme === undefined) {
          throw new Error('deepseek-harness-desktop: advanced shell requires the ui-theme settings namespace')
        }
    ...
    'deepseek-harness-desktop: native shell generation',
  )
}
