...
const BIN_NAME = 'deepseek-harness-desktop'
...
async function start(): Promise<void> {
  ...
  runtime = new ElectronDesktopRuntime(async () => {
    if (shutdown === undefined) {
      throw new Error('deepseek-harness-desktop: shutdown coordinator is not ready')
    }
    ...
    if (profileStartup === undefined || profileStatePath === undefined) {
      throw new Error('deepseek-harness-desktop: renderer boot health arrived before profile startup')
    }
  ...
  try {
    ...
    const ctx = await boot(
      ...
      async (hostCtx) => {
        hostCtx.effect(
          () => releasePnpmRuntime,
          'deepseek-harness-desktop: packaged pnpm runtime PATH',
        )
        if (dshRuntime !== undefined) {
          hostCtx.effect(
            () => releaseDshRuntime,
            'deepseek-harness-desktop: packaged dsh runtime PATH',
          )
        }
        current = hostCtx
        hostCtx.effect(
          () => releasePackageResolver,
          'deepseek-harness-desktop: profile package resolution',
        )
