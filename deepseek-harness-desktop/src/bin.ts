...
export const DESKTOP_CLI_HELP = `Usage: deepseek-harness-desktop [options]
...
async function launchElectron(): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(
      'deepseek-harness-desktop: electron is not available in this installation.\n'
      + 'Install the desktop launcher globally (npm installs the electron peer automatically):\n'
      + '  npm install -g deepseek-harness-desktop\n'
...
export async function runDesktopCli(argv: readonly string[]): Promise<number> {
  ...
  try {
    ...
    process.stderr.write(`deepseek-harness-desktop: ${cause instanceof Error ? cause.message : String(cause)}\n`)
...
if (invokedPath === fileURLToPath(import.meta.url)) {
  void runDesktopCli(process.argv.slice(2)).then(
    ...
    cause => {
      process.stderr.write(`deepseek-harness-desktop: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`)
      process.exitCode = 1
    },
  )
}
