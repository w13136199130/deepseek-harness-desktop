...
function takeDefaultProfile(environment: NodeJS.ProcessEnv): string | undefined {
  ...
  for (const key of Object.keys(environment)) {
    ...
    if (value !== undefined && profileName !== undefined && value !== profileName) {
      throw new Error('deepseek-harness-desktop: conflicting default profile environment values')
    }
    profileName ??= value
...
if (isDirectExecution()) {
  void runDesktopDshCli().catch((cause: unknown) => {
    process.stderr.write(`deepseek-harness-desktop: failed to start packaged dsh: ${cause instanceof Error ? cause.stack ?? cause.message : String(cause)}\n`)
    process.exitCode = 1
  })
}
