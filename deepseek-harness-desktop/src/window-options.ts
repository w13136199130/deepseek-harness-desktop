...
export function compatibilityWindowOptions(
  ...
  if (spec.mode !== 'compatibility') {
    throw new Error(`deepseek-harness-desktop: unsupported compatibility window mode ${spec.mode}`)
  }
...
export function advancedWindowOptions(
  ...
  if (spec.mode !== 'advanced') {
    throw new Error(`deepseek-harness-desktop: unsupported advanced window mode ${spec.mode}`)
  }
  const options: BrowserWindowConstructorOptions = {
    title: platform === 'win32' ? spec.windowTitle : '',
    width: spec.width,
