...
export function desktopProductVersion(moduleUrl: string = import.meta.url): string {
  ...
  if (value === null || typeof value !== 'object' || typeof (value as { version?: unknown }).version !== 'string') {
    throw new Error('deepseek-harness-desktop: package.json has no product version')
  }
...
export class ElectronDesktopRuntime implements DesktopRuntime {
  ...
  schedule(spec: DesktopShellSpec): () => Promise<void> {
    if (this.scheduled !== undefined || this.mountTask !== undefined) {
      throw new Error('deepseek-harness-desktop: a native shell generation is already registered')
    }
  ...
  mountScheduled(beforeInteractive?: () => void): Promise<void> {
    ...
    if (spec === undefined) {
      return Promise.reject(new Error('deepseek-harness-desktop: the Cordis shell plugin did not register a window'))
    }
  ...
  configureTerminal(spec: DesktopTerminalSpec): void {
    if (this.terminalSpec !== undefined) {
      throw new Error('deepseek-harness-desktop: terminal profile is already configured')
    }
  ...
  openTerminal(): void {
    try {
      ...
      if (spec === undefined) {
        throw new Error('deepseek-harness-desktop: terminal profile is not configured')
      }
      const electronVersion = process.versions.electron
      if (electronVersion === undefined) {
        throw new Error('deepseek-harness-desktop: terminal requires the Electron runtime version')
      }
      openDesktopTerminal({
  ...
  reportRendererBoot(report: RendererBootReport): void {
    ...
    try {
      ...
      process.stderr.write(`deepseek-harness-desktop: failed to persist renderer boot health: ${cause instanceof Error ? cause.message : String(cause)}\n`)
    }
    if (report.status === 'failed') {
      void this.showRendererBootRecovery(report).catch((cause: unknown) => {
        process.stderr.write(`deepseek-harness-desktop: failed to show plugin recovery: ${cause instanceof Error ? cause.message : String(cause)}\n`)
      })
    }
  }
  ...
  private async showRendererBootRecovery(report: Extract<RendererBootReport, { status: 'failed' }>): Promise<void> {
    ...
    const result = await dialog.showMessageBox({
      ...
      detail: `Failed plugins:\n${plugins}\n\n${error}\n\nOpen DSH Terminal to update or remove the failing third-party plugin, then restart DeepSeek Harness Desktop.`,
  ...
  private trayCommand(invoke: () => void | Promise<void>): () => void {
    return () => {
      void Promise.resolve().then(invoke).catch((cause: unknown) => {
        process.stderr.write(`deepseek-harness-desktop: tray command failed: ${cause instanceof Error ? cause.message : String(cause)}\n`)
      })
    }
  }
  ...
  private async downloadAndOpenUpdate(version: string, signal: AbortSignal): Promise<void> {
    if (this.platform !== 'darwin' && this.platform !== 'win32') {
      throw new Error(`deepseek-harness-desktop: updates are unavailable on ${this.platform}`)
    }
    ...
    if (this.platform === 'darwin') {
      ...
      if (openError !== '') throw new Error(`deepseek-harness-desktop: failed to open update disk image: ${openError}`)
      signal.throwIfAborted()
      await dialog.showMessageBox({
        type: 'info',
    ...
    const spec = this.scheduled
    if (spec === undefined) throw new Error('deepseek-harness-desktop: no active shell can exit for update installation')
    signal.throwIfAborted()
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      child.once('spawn', () => {
        ...
        child.once('error', cause => {
          process.stderr.write(`deepseek-harness-desktop: update installer failed after launch: ${cause.message}\n`)
        })
        child.unref()
        resolve()
      })
  ...
  private reportTerminalLaunchError(cause: unknown): void {
    ...
    process.stderr.write(`deepseek-harness-desktop: failed to open terminal: ${error.message}\n`)
    try {
      dialog.showErrorBox('Unable to Open DSH Terminal', error.message)
    } catch (dialogCause) {
      process.stderr.write(`deepseek-harness-desktop: failed to show terminal error: ${dialogCause instanceof Error ? dialogCause.message : String(dialogCause)}\n`)
    }
  }
      ...
      { type: 'separator' },
      {
        ...
        click: () => {
          void spec.requestModeChange(nextDesktopShellMode(spec.mode)).catch((cause: unknown) => {
            process.stderr.write(`deepseek-harness-desktop: failed to change shell mode: ${cause instanceof Error ? cause.message : String(cause)}\n`)
          })
        },
      },
      { type: 'separator' },
  ...
  private async mount(
    ...
    if (icon.isEmpty()) {
      throw new Error(`deepseek-harness-desktop: failed to load application icon ${spec.iconPath}`)
    }
      ...
      try {
        ...
        if (target.protocol === 'https:' || target.protocol === 'http:' || target.protocol === 'mailto:') {
          void shell.openExternal(target.href).catch((cause: unknown) => {
            process.stderr.write(`deepseek-harness-desktop: failed to open external link: ${cause instanceof Error ? cause.message : String(cause)}\n`)
          })
        }
      } catch {
    ...
    if (tray === undefined) {
      throw new Error('deepseek-harness-desktop: native tray did not mount')
    }
