...
async function postRendererBootReport(
  ...
  if (!response.ok) {
    throw new Error(`deepseek-harness-desktop: renderer boot report failed with HTTP ${String(response.status)}`)
  }
}
...
export function startRendererBootReporter(
  ...
  const timer = setTimeout(() => {
    void rendererBootReport(loader)
      ...
      .catch((cause: unknown) => {
        console.error('deepseek-harness-desktop: failed to report renderer boot health', cause)
      })
  }, 0)
  return () => {
    active = false
