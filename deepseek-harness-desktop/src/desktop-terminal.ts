/** Isolated command-line environment launched from the DeepSeek Harness Desktop tray. */
...
/** Platforms with a native terminal launch contract owned by DeepSeek Harness Desktop. */
...
function assertScriptValue(label: string, value: string): void {
  if (value.length === 0) throw new Error(`deepseek-harness-desktop: terminal ${label} must not be empty`)
  if (/[\0\r\n]/.test(value)) {
    throw new Error(`deepseek-harness-desktop: terminal ${label} must not contain NUL or newlines`)
  }
}
...
function escapeBatchText(value: string): string {
  if (/\r|\n/.test(value)) {
    throw new Error('deepseek-harness-desktop: terminal Windows welcome text must not contain newlines')
  }
  return value
    .replaceAll('^', '^^')
    .replaceAll('%', '%%')
...
function prepareStateDirectory(stateDir: string): void {
  ...
  if (!stat.isDirectory() || stat.isSymbolicLink()) {
    throw new Error(`deepseek-harness-desktop: terminal state path is not a private directory: ${stateDir}`)
  }
...
function prepareDesktopTerminalFiles(options: DesktopTerminalOptions): DesktopTerminalFiles {
  if (options.platform !== 'darwin' && options.platform !== 'win32') {
    throw new Error(`deepseek-harness-desktop: terminal is unsupported on ${options.platform}`)
  }
...
function windowsShellArgv(
  ...
  if (shell.kind === 'cmd') {
    ...
    if (welcome === undefined) {
      throw new Error('deepseek-harness-desktop: terminal did not generate the Windows command prompt welcome script')
    }
...
function resolveWindowsCommandProcessor(
  ...
  if (executable === undefined) {
    throw new Error('deepseek-harness-desktop: terminal requires cmd.exe to create a visible Windows console')
  }
...
function reportLaunchError(options: DesktopTerminalOptions, cause: Error): void {
  if (options.onLaunchError !== undefined) {
    try {
      ...
      process.stderr.write(`deepseek-harness-desktop: terminal error reporter failed: ${detail}\n`)
      return
    }
  }
  process.stderr.write(`deepseek-harness-desktop: failed to open terminal: ${cause.message}\n`)
}
