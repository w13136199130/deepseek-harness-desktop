...
export function apply(ctx: Context): void {
  if (ctx.desktopRuntime.platform === 'linux') {
    throw new Error('deepseek-harness-desktop: the packaged terminal is supported on macOS and Windows')
  }
  ctx.effect(() => {
  ...
  }, 'deepseek-harness-desktop: packaged terminal tray command')
}
