...
const seen = new Set()
const manifests = []
const queue = [{ name: rootManifest.name ?? 'deepseek-harness-desktop', manifestPath: join(packageRoot, 'package.json') }]
...
if (noticesArg !== -1) {
  ...
  const lines = [
    '# Third-Party Notices',
    '',
    'DeepSeek Harness Desktop distributes the following third-party packages inside its installers.',
