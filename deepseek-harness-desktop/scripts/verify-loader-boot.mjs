...
const BIN_NAME = 'deepseek-harness-desktop-loader-smoke'
...
try {
  ...
  if (profileRequire.resolve('deepseek-harness-desktop/package.json') !== desktopManifest) {
  ...
  if (profileDirectoryRequire.resolve('deepseek-harness-desktop/package.json') !== desktopManifest) {
  ...
  ctx = await boot(
    ...
    prepared.rootConfig,
    [{ insert: [
      { id: 'desktop-shell', name: 'deepseek-harness-desktop' },
  ...
  if (desktopEntry?.options.name !== 'deepseek-harness-desktop') {
  ...
  if (mountedSpec?.url !== 'http://127.0.0.1:43120/?deepseek-harness-desktop-mode=compatibility&deepseek-harness-desktop-platform=darwin') {
