...
export const REQUIRED_UNPACKED_PACKAGE_SPECIFIERS = [
  ...
  'deepseek-harness-desktop/profile',
  'deepseek-harness-desktop/client',
  'deepseek-harness-desktop/terminal',
  'deepseek-harness-desktop/pnpm',
  'deepseek-harness-desktop/profile-service',
...
export function resolvePackagedAsarPath(context: PackagedRuntimeContext): string {
  ...
  throw new Error(
    `deepseek-harness-desktop: unsupported Electron afterPack platform ${JSON.stringify(context.electronPlatformName)}`,
  )
}
...
export function verifyPackagedAsar(
  ...
  try {
    ...
    throw new Error(
      `deepseek-harness-desktop: failed to inspect packaged runtime at ${archivePath}`,
      { cause },
    )
  }
  ...
  if (missing.length > 0) {
    throw new Error(
      `deepseek-harness-desktop: packaged runtime at ${archivePath} is missing required ASAR entries: ${missing.join(', ')}`,
    )
  }
}
...
export function verifyUnpackedPackageResolution(
  ...
  for (const specifier of REQUIRED_UNPACKED_PACKAGE_SPECIFIERS) {
    ...
    try {
      ...
      throw new Error(
        `deepseek-harness-desktop: packaged runtime at ${unpackedRoot} cannot resolve required package export ${specifier}`,
        { cause },
      )
    }
    ...
    if (
      ...
      throw new Error(
        `deepseek-harness-desktop: required package export ${specifier} resolved outside ${unpackedRoot}: ${resolvedPath}`,
      )
    }
  }
}
...
export function verifyPackagedRuntime(
  ...
  if (missing.length > 0) {
    throw new Error(
      `deepseek-harness-desktop: packaged runtime at ${unpackedRoot} is missing required physical entries: ${missing.join(', ')}`,
    )
  }
